...
class ControladorEntradasCine2 extends Controller
{

    public function guardarNumButaca($butaca, $dia, $hora, $fila, $idEntrada){
        $datos = new Butacas();
        $datos->titulo = 'Harry Potter';
    ...
    public function compraEntradas(Request $request){
        ...
        if ($validator->fails()) {
            ...
            $datos->tituloPelicula = 'Harry Potter';
    ...
    $datos = Butacas::select('numButaca')->where('dia', $dia)->where('hora', $hora)->where('titulo', 'Harry Potter')->get();
    if(empty($datos)){
        $data1 = "";
        $data2 = "";
        $data3= "";
