...
class ControladorEntradasCine3 extends Controller
{

    public function guardarNumButaca($butaca, $dia, $hora, $fila, $idEntrada){
        $datos = new Butacas();
        $datos->titulo = 'Annabelle';
    ...
    public function compraEntradas(Request $request){
        ...
        if ($validator->fails()) {
            ...
            $datos->tituloPelicula = 'Annabelle';
    ...
    $datos = Butacas::select('numButaca')->where('dia', $dia)->where('hora', $hora)->where('titulo', 'Annabelle')->get();
    if(empty($datos)){
        $data1 = "";
        $data2 = "";
        $data3= "";
