...
<style>
    body{
        margin: 0;
        padding: 0;
        background-image: url('Imagenes/fondojoker2.jpg');
        ...
        function sumarIdEntrada(){
            ...
            for(var j=0; j<241; j++){
                if($(`#butaca${j}`).hasClass('clicked')){
                    for(var n=0; n<21; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '1' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=21; n<41; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '2' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=41; n<61; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '3' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=61; n<81; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '4' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=81; n<101; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '5' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=101; n<121; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '6' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=121; n<141; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '7' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=141; n<161; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '8' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=161; n<181; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '9' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=181; n<201; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '10' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=201; n<221; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/' + '11' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                        for(var n=221; n<241; n++){
                            if(n==`${j}`){
                                $.get('guardarNumButaca/'+j +'/'+dia+'/'+hora+'/'+'12' + '/' + idEntrada, function(response) {
                                })
                            }
                        }
                }
...
<body>
    ...
    <form id="form" method="POST" action="{{ route('compraEntradas') }}">
        ...
        <div class="formulario">
            ...
            <div class="ticket">
                <div class="borde"></div>
                <center><div class="titulo2"><h2>JOKER</h2></div></center>
